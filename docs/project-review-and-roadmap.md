...
> **Implementation status (July 12, 2026):** P2-A and P2-B landed. Progressive results: `runClientScan` streams rows/errors via `onRow`/`onError` callbacks and the home page upserts them into the table as each ticker completes. Cancel: the Clear button becomes an enabled "Cancel scan" while loading; cancellation is graceful (no new requests start, in-flight fetches abort, completed rows are KEPT, and a status line reports "showing N of M"). Retry: a "Retry failed (n)" button re-fetches only transiently-failed tickers (RATE_LIMITED/PROVIDER_ERROR — not INVALID_TICKER/NOT_FOUND) and merges results into the existing table. Filters: the tested filter engine is back in the UI (`components/FilterBar.tsx`) with the focused evidence-aware set — Strength ≥, Risk ≤, coverage ≥, industry, market cap, P/E, dividend yield, include-unavailable — with active-filter chips, per-chip clear, and an n-of-m match count; filtering is display-only and never re-fetches. Share links now carry filter state with a schema version (`v=2`; future-version links keep tickers and drop uninterpretable filter params). Mobile: the identity column is sticky under 700px so horizontal scrolling keeps the company visible. Still open from this group: column presets, stacked mobile triage cards, and hands-on assistive-tech/browser verification (this pass was verified by unit tests, typecheck, build, and a live API scan — not by hands-on mobile/AT testing).
